...
/**
 * TC_FBS_011 — FBS (Fulfilled By Store) order lifecycle, 2 products (qty=1 each) from
 * 2 DIFFERENT 3P sellers, PayPal payment.
 *
 * Combines TC_FBS_005's dual-shipment flow (2 different sellers → 2 Mirakl shipments, WEB-A +
 * WEB-B, each independently run through Accept -> Kibo deliveryType check -> Documents ->
 * Tracking -> Mark as Shipped -> Entregado -> Received) with TC_FBS_007/008/009/010's PayPal
 * payment flow. Different-seller splitting is structural in Mirakl (confirmed across every
 * 2-different-seller FBS/FBO test in this codebase), not a quantity-driven assumption.
 *
 * NOTE on SKUs: the manual test case's supplied SKU pair (32982398137/32982398136) is the
 * confirmed SAME-SELLER pairing already used by TC_FBS_002/004/008/010 — with those SKUs this
 * order would land as a single Mirakl shipment, contradicting the case's "two 3P Seller" title.
 * Per explicit user confirmation, this TC instead uses the known different-seller pairing
 * (32982398137 + 4892828006, same as TC_FBS_005) so it actually exercises the dual-shipment path
 * the title describes.
 *
 * Step numbers in the log/comments below are 1:1 with the 73-step manual test case; the PayPal
 * payment section is exactly one step longer than TC_FBS_005's Credit Card section (extra
 * "PayPal Pagar" click), so everything from "Proceed to payment" onward is shifted +1 vs
 * TC_FBS_005's step numbers. The Mirakl/Kibo/Documents/Tracking/Shipped/Entregado phase (run
 * independently per shipment via runFbsFulfillmentFlow) is copied from TC_FBS_005 essentially
 * verbatim, including its generic-search-once-per-attempt fix (see CLAUDE.md) and the 10s
 * inter-retry sleep in waitForMiraklStatus (see TC_FBS_007's Entregado->Received timing fix).
 *
 ...
 */
public class TC_FBS_011_Test extends BaseClass {

    private static final String TC_NAME = "TC_FBS_011";
    ...
    // PayPal sandbox test data — same sandbox account already used by TC_FBO_007/TC_FBS_007-010
    ...
    public void initPageObjects() {
        ...
        LoggerUtility.info("All page objects initialized for TC_FBS_011");
    }

    @Test(testName = TC_NAME, groups = {"FBS"},
          description = "Verify FBS order placement (2 products qty=1 each, 2 different 3P sellers, PayPal payment) and full fulfilment lifecycle for BOTH resulting Mirakl shipments (WEB-A + WEB-B): FDA -> Mirakl Accept -> Kibo (FBS) -> Documents/Tracking -> Shipped -> Received")
    public void tc_fbs_011_place_order_2products_diff_sellers_paypal_fbs_full_fulfillment() throws InterruptedException {
        ...
        // ============================================================
        // PHASE 1: FDA — Search, PDP, Cart (2 products, 2 different sellers), PayPal Payment, Order
        // ============================================================

        switchToFDATab();
        LoggerUtility.info("TC_FBS_011: Starting — FDA session active from @BeforeGroups(FBS), Mirakl session active from suite setup");
        ...
        String sku1 = config.getFbs011Sku1();
        String sku2 = config.getFbs011Sku2();
        ...
        // Step 31 (manual case): verify order total for both products is shown on the "Completar
        // pago" button — that button/text only exists on the Credit Card path (TC_FBS_005); the
        // live-confirmed PayPal flow (TC_FBS_007/008/009/010) has no equivalent element on
        // PayPal's review page, so this logs the cart total captured in Step 20 instead of
        // asserting on an element that was never observed to exist for PayPal orders.
        LoggerUtility.info("Step 31: Order total for both products (captured in cart): " + orderTotal
                + " — PayPal review page has no equivalent 'Completar pago (MXN$...)' button to re-verify this against");

        // Step 32: Click Compra completa — this is the button that actually finalizes the
        // PayPal-side authorization (same button TC_FBO_007/TC_FBS_007-010 click); it closes the
        ...
        LoggerUtility.info("Step 32: Clicking Compra completa button on PayPal review page");
        ...
        // Steps 33-34: Success page → get order ID
        ...
        LoggerUtility.info("Step 39: FDA order status: " + fdaStatus + " (expected 'Creada')");
        // PayPal orders can land on "Procesando" instead of "Creada"/"Pendiente" — same tolerance
        // TC_FBS_007-010's PayPal flow already applies; the Credit Card FBS tests don't see this
        ...
        // case's remaining steps (40-73) are applied independently to each shipment below.
        ...
        // technique TC_FBS_005/TC_FBO_005 use for 2-different-seller orders; unlike same-seller
        ...
        LoggerUtility.info("TC_FBS_011 execution completed successfully — WEB-A and WEB-B both Received");
    ...
    // TC_FBS_002-010) and return its detail page URL for later direct navigation.
    // ----------------------------------------------------------------
    ...
    // number; the same invoice PDF (fbs011.invoice.file.path) is uploaded for both shipments.
    // ----------------------------------------------------------------
    private void runFbsFulfillmentFlow(String shipmentRef, String miraklDetailUrl) throws InterruptedException {
        ...
        LoggerUtility.info("Uploading invoice PDF: " + config.getFbs011InvoiceFilePath() + " [" + shipmentRef + "]");
        miraklOrderDetailPage.uploadDocumentFile(config.getFbs011InvoiceFilePath());
    ...
    // retries (same fix TC_FBO_030/TC_FBS_007-010 already apply for slow post-acceptance status
