...
public class TC_FBO_012_Test extends BaseClass {

    private static final String TC_NAME = "TC_FBO_012";
    ...
    @Test(testName = TC_NAME, groups = {"FBO"},
          description = "Verify 2 products qty=2 each from 2 different 3P sellers with PayPal payment → 2 Mirakl shipments → both Received")
    public void tc_fbo_012_two_products_qty2_paypal_two_sellers_full_fulfillment() throws InterruptedException {

        LoggerUtility.info(TC_NAME + " execution started");

        // ============================================================
        // PHASE 1: FDA — Search 2 SKUs (qty=2 each), Cart, PayPal, Order
        // ============================================================
        ...
        // Wait for async UI update before reading quantity
        driver.manage().timeouts().implicitlyWait(Duration.ZERO);
        try {
            new WebDriverWait(driver, Duration.ofSeconds(10))
                .until(d -> "2".equals(fdaPdpPage.getQuantity()));
        } catch (Exception e) {
            LoggerUtility.warn(TC_NAME + ": Qty not updated to 2 for SKU 1 — retrying click. " + e.getMessage());
            fdaPdpPage.increaseQuantity();
        } finally {
        ...
        Assert.assertEquals(pdpQty1, "2",
            TC_NAME + ": Product 1 PDP quantity should be 2. Actual: " + pdpQty1);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);
        ...
        try {
            ...
            LoggerUtility.warn(TC_NAME + ": Qty not updated to 2 for SKU 2 — retrying click. " + e.getMessage());
            fdaPdpPage.increaseQuantity();
        } finally {
            driver.manage().timeouts().implicitlyWait(Duration.ofMinutes(2));
        }
        ...
        Assert.assertEquals(pdpQty2, "2",
            TC_NAME + ": Product 2 PDP quantity should be 2. Actual: " + pdpQty2);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);
        ...
        // Quick cart check — both items must be present before full cart validation
        driver.manage().timeouts().implicitlyWait(Duration.ZERO);
        ...
        if (!cartHas2Items) {
            ...
            fdaPdpPage.clickAddToCart();
            LoggerUtility.info(TC_NAME + ": Retry add-to-cart for SKU 2 (qty=2) complete");
        }

        // ---- Steps 18-22: Cart Validation ----
        LoggerUtility.info("Step 18: Opening cart page");
        ...
        LoggerUtility.info("Step 21: Cart quantities: " + quantities);
        ...
        Assert.assertEquals(quantities.get(0), "2",
            TC_NAME + ": Product 1 cart quantity should be 2. Actual: " + quantities.get(0));
        Assert.assertEquals(quantities.get(1), "2",
            TC_NAME + ": Product 2 cart quantity should be 2. Actual: " + quantities.get(1));
        ...
        // ---- Steps 23-26: Checkout + PayPal Selection ----
        ...
        // ---- Step 27: Switch to PayPal popup ----
        LoggerUtility.info("Step 27: Detecting and switching to PayPal popup window");
        switchToPayPalWindow(handlesBeforePayPal);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // ---- Steps 28-34: PayPal Login + Complete ----
        fdaPayPalPage.waitForPageLoad();
/*
        LoggerUtility.info("Step 28: Clicking PayPal email field");
        fdaPayPalPage.clickEmailField();

        LoggerUtility.info("Step 29: Entering PayPal email (masked in logs)");
        fdaPayPalPage.enterEmail(TC_PAYPAL_EMAIL);

        LoggerUtility.info("Step 30: Clicking 'Siguiente' on PayPal");
        fdaPayPalPage.clickNextButton();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 31: Clicking 'Contraseña' field on PayPal");
        fdaPayPalPage.clickPasswordField();

        LoggerUtility.info("Step 32: Entering PayPal password (masked in logs)");
        fdaPayPalPage.enterPassword(TC_PAYPAL_PASS);

        LoggerUtility.info("Step 33: Clicking 'Iniciar sesión' on PayPal");
        fdaPayPalPage.clickLoginButton();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);
*/
        LoggerUtility.info("Step 34: Clicking 'Compra completa' on PayPal review page");
        fdaPayPalPage.clickCompletePayment();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // ---- Step 35: PayPal window closed, back to FDA ----
        LoggerUtility.info("Step 35: Waiting for PayPal window to close and switching back to FDA");
        waitForPayPalWindowClose();
        LoggerUtility.info("Switched back to FDA checkout/success window");

        // ---- Steps 36-37: Success Page + Order ID ----
        LoggerUtility.info("Step 36: Verifying FDA order success page");
        ...
        // ---- Steps 38-42: Order History Validation ----
        ...
        LoggerUtility.info("Step 42: FDA order status: " + fdaStatus);
        Assert.assertTrue(
        ...
        LoggerUtility.info("Step 43: Switching to Mirakl tab");
        switchToMiraklTab();
        ...
        LoggerUtility.info("Step 45: Clicking All Orders");
        ...
        LoggerUtility.info("Step 46-47: Searching for order: " + miraklSearchTerm);
        ...
        // Step 48: Verify exactly 2 shipments (WEB-A + WEB-B)
        LoggerUtility.info("Step 48: Verifying 2 shipments — WEB-A: " + shipmentRefA + " | WEB-B: " + shipmentRefB);
        ...
        // Step 49: Accept WEB-A
        LoggerUtility.info("Step 49: Opening and accepting WEB-A shipment: " + shipmentRefA);
        ...
        // Accept WEB-B
        LoggerUtility.info("Step 49: Opening and accepting WEB-B shipment: " + shipmentRefB);
        ...
        // Step 50: Wait 2 minutes after both acceptances
        LoggerUtility.info("Step 50: Waiting 2 minutes after acceptance for orders to stabilize in Kibo...");
        ...
        LoggerUtility.info("Step 51: ========== Kibo Authentication ==========");
        ...
        LoggerUtility.info("Step 52: ========== Find Kibo Order ID ==========");
        ...
        LoggerUtility.info("Step 53: ========== Polling Kibo Shipments (2 expected) ==========");
        ...
        // Steps 54-63: Process each shipment independently
        for (ShipmentInfo info : shipments) {
            LoggerUtility.info("Step 54: ========== Processing Shipment: " + info.shipmentRef + " ==========");
            LoggerUtility.info("Delivery Partner: " + info.deliveryPartner);
    ...
    private void runEnvioclickFlow(ShipmentInfo info) {
        ...
        LoggerUtility.info("Step 56: Checking Mirakl for Shipped/3PL delivery status — " + info.shipmentRef);
        ...
        LoggerUtility.info("Step 58: Checking Mirakl for Received status — " + info.shipmentRef);
    ...
    private void runSkydropxFlow(ShipmentInfo info) {
        ...
        LoggerUtility.info("Step 60: Checking Mirakl for Shipped status — " + info.shipmentRef);
        ...
        LoggerUtility.info("Step 62: Checking Mirakl for Received status — " + info.shipmentRef);
        status = waitForMiraklStatus("Received", 20);
