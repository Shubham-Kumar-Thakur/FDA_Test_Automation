...
public class TC_FBO_008_Test extends BaseClass {

    private static final String TC_NAME = "TC_FBO_008";
    ...
    private static final String TC_SKU_2        = "7080901020305";
    ...
    @Test(testName = TC_NAME, groups = {"FBO"},
          description = "Verify 2 products qty=1 each from 1 3P seller with PayPal payment → 2 Mirakl shipments → both Received")
    public void tc_fbo_008_two_products_qty1_paypal_one_seller_full_fulfillment() throws InterruptedException {
        ...
        LoggerUtility.info("Step 18: Cart product names: " + cartProductNames);
        ...
        LoggerUtility.info("Step 19: Cart quantities: " + quantities);
        ...
        // ---- Steps 21-23: Checkout + PayPal Selection ----
        ...
        LoggerUtility.info("Step 23: Selecting 'PayPal' radio button");
        ...
        // ---- Step 25: Switch to PayPal popup ----
        LoggerUtility.info("Step 25: Detecting and switching to PayPal popup window");
        switchToPayPalWindow(handlesBeforePayPal);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // ---- Steps 26-32: PayPal Login + Complete ----
        fdaPayPalPage.waitForPageLoad();
        /*
        LoggerUtility.info("Step 26: Clicking PayPal email field");
        fdaPayPalPage.clickEmailField();

        LoggerUtility.info("Step 27: Entering PayPal email (masked in logs)");
        fdaPayPalPage.enterEmail(TC_PAYPAL_EMAIL);

        LoggerUtility.info("Step 28: Clicking 'Siguiente' on PayPal");
        fdaPayPalPage.clickNextButton();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 29: Clicking 'Contraseña' field on PayPal");
        fdaPayPalPage.clickPasswordField();

        LoggerUtility.info("Step 30: Entering PayPal password (masked in logs)");
        fdaPayPalPage.enterPassword(TC_PAYPAL_PASS);

        LoggerUtility.info("Step 31: Clicking 'Iniciar sesión' on PayPal");
        fdaPayPalPage.clickLoginButton();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);
*/
        LoggerUtility.info("Step 32: Clicking 'Compra completa' on PayPal review page");
        fdaPayPalPage.clickCompletePayment();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // ---- Step 33: PayPal window closed, back to FDA ----
        LoggerUtility.info("Step 33: Waiting for PayPal window to close and switching back to FDA");
        waitForPayPalWindowClose();
        LoggerUtility.info("Switched back to FDA checkout/success window");

        // ---- Step 34-35: Success Page + Order ID ----
        LoggerUtility.info("Step 34: Verifying FDA order success page");
        ...
        // ---- Steps 36-40: Order History Validation ----
        ...
        LoggerUtility.info("Step 41: Switching to Mirakl tab");
        switchToMiraklTab();
        ...
        LoggerUtility.info("Step 44-45: Searching for order: " + miraklSearchTerm);
        ...
        // Step 46: Verify exactly 2 shipments (WEB-A + WEB-B)
        LoggerUtility.info("Step 46: Verifying 2 shipments — WEB-A: " + shipmentRefA + " | WEB-B: " + shipmentRefB);
        ...
        LoggerUtility.info("Step 47: Opening and accepting WEB-A shipment: " + shipmentRefA);
        ...
        // Accept WEB-B
        LoggerUtility.info("Step 47: Opening and accepting WEB-B shipment: " + shipmentRefB);
        ...
        // Step 48: Wait 2 minutes after both acceptances
        LoggerUtility.info("Step 48: Waiting 2 minutes after acceptance for orders to stabilize in Kibo...");
            ...
            if (shipmentList != null && shipmentList.size() >= 2) {
                ...
                for (Map<String, Object> s : shipmentList) {
                    ...
                    if (info.tplShipmentId.isEmpty() || info.deliveryPartner.isEmpty()) {
                        ...
                        // For Envioclick, also wait for trackingNumber
        ...
        // Step 52-61: Process each shipment independently
        for (ShipmentInfo info : shipments) {
            LoggerUtility.info("Step 52: ========== Processing Shipment: " + info.shipmentRef + " ==========");
            LoggerUtility.info("Delivery Partner: " + info.deliveryPartner);
    ...
    private void runEnvioclickFlow(ShipmentInfo info) {
        ...
        LoggerUtility.info("Step 54: Checking Mirakl for Shipped/3PL delivery status — " + info.shipmentRef);
        ...
        LoggerUtility.info("Step 56: Checking Mirakl for Received status — " + info.shipmentRef);
    ...
    private void runSkydropxFlow(ShipmentInfo info) {
        ...
        LoggerUtility.info("Step 58: Checking Mirakl for Shipped status — " + info.shipmentRef);
        ...
        LoggerUtility.info("Step 60: Checking Mirakl for Received status — " + info.shipmentRef);
        status = waitForMiraklStatus("Received", 20);
