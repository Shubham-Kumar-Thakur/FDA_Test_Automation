...
public class TC_FBO_022_Test extends BaseClass {

    private static final String TC_NAME        = "TC_FBO_022";

    // TC-specific test data — mgowda account, 1 SKU qty=2, single 3P seller
    ...
    public void initAndLogin() {
        ...
        if (!TC_FDA_USER.equals(config.getFdaUsername())) {
            ...
            LoggerUtility.info("TC_FBO_022: FDA login as " + TC_FDA_USER + " successful");
        } else {
            LoggerUtility.info("TC_FBO_022: Suite user already " + TC_FDA_USER + " — no session swap needed");
        }
    }
    ...
    public void restoreOriginalFdaSession() {
        if (!TC_FDA_USER.equals(config.getFdaUsername())) {
            LoggerUtility.info("TC_FBO_022 @AfterClass: Restoring original FDA session to "
            ...
            try {
                ...
                LoggerUtility.info("TC_FBO_022 @AfterClass: FDA session restored as " + config.getFdaUsername());
            } catch (Exception e) {
                LoggerUtility.error("TC_FBO_022 @AfterClass: Failed to restore FDA session: " + e.getMessage());
            }
        } else {
            LoggerUtility.info("TC_FBO_022 @AfterClass: No session restore needed — suite user unchanged");
        }
    }
    ...
    @Test(testName = TC_NAME, groups = {"FBO"},
          description = "Verify 1 SKU qty=2 from 1 3P seller → 1 Mirakl shipment (WEB-A) → full fulfillment → return → compliance → full refund → Closed")
    public void tc_fbo_022_single_product_qty2_return_refund() throws InterruptedException {

        LoggerUtility.info("TC_FBO_022 execution started");
        LoggerUtility.info("Scenario: 1 SKU qty=2 from 3P seller → WEB-A → Received → Return → Closed");
        LoggerUtility.info("Test Case ID      : " + TC_NAME);
        LoggerUtility.info("SKU               : " + TC_SKU);
        LoggerUtility.info("Quantity          : " + TC_QUANTITY);
        ...
        LoggerUtility.info("Step 7: Clicking + button once to increase quantity from 1 to " + TC_QUANTITY);
        fdaPdpPage.increaseQuantity();

        // Step 8: Verify PDP quantity = 2
        String pdpQty = fdaPdpPage.getQuantity();
        LoggerUtility.info("Step 8: Verifying PDP quantity is exactly " + TC_QUANTITY + ". Actual: " + pdpQty);
        Assert.assertEquals(pdpQty, String.valueOf(TC_QUANTITY),
            "PDP quantity should be " + TC_QUANTITY + " | SKU: " + TC_SKU + " | TC: " + TC_NAME);
        LoggerUtility.info("PDP quantity confirmed = " + TC_QUANTITY);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // Step 9: Add to cart
        LoggerUtility.info("Step 9: Clicking Agregar al carrito");
        fdaPdpPage.clickAddToCart();
        LoggerUtility.info("SKU " + TC_SKU + " added to cart — qty=" + TC_QUANTITY);
        ...
        Assert.assertEquals(cartQty, String.valueOf(TC_QUANTITY),
            "Cart quantity should be " + TC_QUANTITY + " | SKU: " + TC_SKU + " | TC: " + TC_NAME);
        LoggerUtility.info("Cart quantity verified = " + TC_QUANTITY);
        ...
        LoggerUtility.info("Order created successfully — Order ID: " + orderId
            + " | SKU: " + TC_SKU + " | qty=" + TC_QUANTITY + " | total=" + orderTotal);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);
        ...
        // ============================================================
        // PHASE 5: MIRAKL — Find WEB-A and Accept
        // ============================================================
        ...
        // Steps 36-37: Search — retry every 60s up to 10 minutes for Mirakl sync delay
        ...
        LoggerUtility.info("Exactly 1 shipment verified — WEB-A for SKU: " + TC_SKU + " qty=" + TC_QUANTITY);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // Step 39: Open WEB-A detail, verify status, Accept
        ...
        // Step 55: Create Return incident via API
        ...
        LoggerUtility.info("Calling Return service API — creating return incident");
        ...
        // Step 56: Switch to Mirakl and navigate to WEB-A order detail
        LoggerUtility.info("Step 56: Switching to Mirakl — navigating to order detail for return");
        switchToMiraklTab();
        miraklOrdersPage.clickOrdersMenu();
        miraklOrdersPage.clickAllOrders();
        ...
        // Step 61: Select "Full refund"
        LoggerUtility.info("Step 61: Selecting Full refund from dropdown");
        miraklReturnPage.selectFullRefundFromDropdown();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // Step 62: Click "Select" dropdown for refund reason
        ...
        // Step 64: Click "Confirm" on refund popup
        LoggerUtility.info("Step 64: Clicking Confirm on refund popup");
        miraklReturnPage.confirmRefundPopup();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // Step 65: Wait 1 minute for refund to process, then refresh
        LoggerUtility.info("Step 65: Waiting 60 seconds for shipment closure to process...");
        Thread.sleep(60_000);
        LoggerUtility.info("60-second wait complete");
        LoggerUtility.info("Step 65: Refreshing Mirakl page");
        refreshAndWait(MIRAKL_ORDER_STATUS_LOCATOR);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        // Step 66: Verify Closed
        LoggerUtility.info("===== PHASE 10: Step 66 — Verify Shipment WEB-A = Closed =====");
        String closedStatus = waitForMiraklStatus("Closed", 6);
        LoggerUtility.info("Step 66: Shipment WEB-A status after full refund: " + closedStatus);
        ...
        // Final summary log
        LoggerUtility.info("TC_FBO_022 completed successfully");
        LoggerUtility.info("  Test Case         : TC_FBO_022");
        LoggerUtility.info("  SKU               : " + TC_SKU);
        LoggerUtility.info("  Quantity          : " + TC_QUANTITY);
        LoggerUtility.info("  Expected Total    : MXN$800.00");
