...
/**
 * TC_FBS_002 — FBS (Fulfilled By Store) order lifecycle, 2 products (qty=1 each) from 1 3P seller:
 ...
 *
 * Step numbers in the log/comments below are 1:1 with the 72-step manual test case. The
 * Mirakl/Kibo/Documents/Tracking/Mark-as-Shipped/Entregado mechanics are copied verbatim from
 * the already-verified TC_FBS_001_Test — only the FDA phase differs (2 products added to cart
 * instead of 1). Both products belong to the same 3P seller, so this produces a single Mirakl
 * shipment (no WEB-A/WEB-B split), same as TC_FBS_001.
 *
 ...
 */
public class TC_FBS_002_Test extends BaseClass {

    private static final String TC_NAME = "TC_FBS_002";
    ...
    public void initPageObjects() {
        ...
        LoggerUtility.info("All page objects initialized for TC_FBS_002");
    }

    @Test(testName = TC_NAME, groups = {"FBS"},
          description = "Verify FBS order placement (2 products, 1 3P seller) and full fulfilment lifecycle: FDA -> Mirakl Accept -> Kibo (FBS) -> Documents/Tracking -> Shipped -> Received")
    public void tc_fbs_002_place_order_2products_fbs_full_fulfillment() throws InterruptedException {
        ...
        switchToFDATab();
        LoggerUtility.info("TC_FBS_002: Starting — FDA session active from @BeforeGroups(FBS), Mirakl session active from suite setup");
        ...
        String sku1 = config.getFbs002Sku1();
        String sku2 = config.getFbs002Sku2();
        ...
        // Confirmed via live run (2026-09-07): this shop's orders can already be auto-accepted by
        // the time the 5-minute Mirakl sync window elapses, landing directly on "Awaiting shipment"
        // instead of "Pending acceptance" — same auto-accept behavior MiraklOrderDetailPage's
        // isAcceptButtonPresent() was written to detect elsewhere in this codebase. Tolerate both.
        ...
        LoggerUtility.info("Step 58: Selecting the invoice PDF file: " + config.getFbs002InvoiceFilePath());
        miraklOrderDetailPage.uploadDocumentFile(config.getFbs002InvoiceFilePath());
        ...
        // TC_FBS_001 live run) Clicking More actions > Documents navigates the browser away from
        // the Order Detail page to a separate "Order documents" page. "Add tracking information"
        // and "Mark as Shipped" only exist on the Order Detail page, so we must navigate back the
        // same way we got here the first time (Steps 39-40, 44) before Step 60 can find anything.
        ...
        // Step 66: Verify the selected Carrier Name and Tracking Number are displayed — confirmed
        // via the TC_FBS_001 live run that this requires opening "View tracking information" and
        // reading the values from the resulting dialog (then closing it).
        ...
        // (Confirmed necessary by the TC_FBS_001 live run, not spelled out as a separate numbered
        // step in this manual test case) Entregado has a Yes/No value dropdown that must be set
        // before Confirm does anything meaningful.
        ...
        LoggerUtility.info("TC_FBS_002 execution completed successfully");
