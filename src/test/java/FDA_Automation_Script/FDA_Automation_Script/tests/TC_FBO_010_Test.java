...
public class TC_FBO_010_Test extends BaseClass {

    private static final String TC_NAME = "TC_FBO_010";
    ...
    @Test(testName = TC_NAME, groups = {"FBO"},
          description = "Verify 2 products qty=2 each from 1 3P seller with PayPal payment → 2 Mirakl shipments → both Received")
    public void tc_fbo_010_two_products_qty2_paypal_one_seller_full_fulfillment() throws InterruptedException {

        LoggerUtility.info(TC_NAME + " execution started");

        // ============================================================
        // PHASE 1: FDA — 2 Products qty=2 each, PayPal, Order
        // ============================================================
        ...
        LoggerUtility.info("Step 7: Clicking 'plus' button to increase Product 1 quantity to 2");
        fdaPdpPage.increaseQuantity();

        // Wait for qty to update asynchronously
        driver.manage().timeouts().implicitlyWait(Duration.ZERO);
        try {
            new WebDriverWait(driver, Duration.ofSeconds(10))
                .until(d -> "2".equals(fdaPdpPage.getQuantity()));
        } catch (Exception e) {
            LoggerUtility.warn(TC_NAME + ": Qty not updated to 2 after increaseQuantity — retrying for Product 1. " + e.getMessage());
            fdaPdpPage.increaseQuantity();
        } finally {
        ...
        LoggerUtility.info("Step 8: Product 1 PDP quantity: " + pdpQty1);
        Assert.assertEquals(pdpQty1, "2",
            TC_NAME + ": Product 1 PDP quantity should be 2 after clicking plus. Actual: " + pdpQty1);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 9: Clicking 'Agregar al carrito' for Product 1");
        fdaPdpPage.clickAddToCart();
        LoggerUtility.info("Product 1 (qty=2) added to cart");
        ...
        LoggerUtility.info("Step 13: Verifying PDP for Product 2");
        Assert.assertTrue(fdaPdpPage.isDisplayed(),
            TC_NAME + ": PDP not displayed for SKU 2: " + TC_SKU_2);

        LoggerUtility.info("Step 14: Verifying 'Agregar al carrito' button enabled for Product 2");
        Assert.assertTrue(fdaPdpPage.isAddToCartEnabled(),
            TC_NAME + ": Add to cart button not enabled for SKU 2: " + TC_SKU_2);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 15: Clicking 'plus' button to increase Product 2 quantity to 2");
        fdaPdpPage.increaseQuantity();
        ...
        try {
            ...
            LoggerUtility.warn(TC_NAME + ": Qty not updated to 2 after increaseQuantity — retrying for Product 2. " + e.getMessage());
            fdaPdpPage.increaseQuantity();
        } finally {
            driver.manage().timeouts().implicitlyWait(Duration.ofMinutes(2));
        }
        ...
        LoggerUtility.info("Step 16: Product 2 PDP quantity: " + pdpQty2);
        Assert.assertEquals(pdpQty2, "2",
            TC_NAME + ": Product 2 PDP quantity should be 2 after clicking plus. Actual: " + pdpQty2);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 17: Clicking 'Agregar al carrito' for Product 2");
        fdaPdpPage.clickAddToCart();
        LoggerUtility.info("Product 2 (qty=2) added to cart");

        // Quick cart check — both items must be present
        driver.manage().timeouts().implicitlyWait(Duration.ZERO);
        ...
        try {
            ...
            LoggerUtility.warn(TC_NAME + ": Cart has fewer than 2 items — retrying SKU 2 add. " + e.getMessage());
        } finally {
            driver.manage().timeouts().implicitlyWait(Duration.ofMinutes(2));
        }
        if (!cartHas2Items) {
            LoggerUtility.info(TC_NAME + ": Retrying add-to-cart for SKU 2 with qty=2");
            fdaHomePage.navigateTo(config.getFdaUrl());
            fdaHomePage.enterSearchQuery(TC_SKU_2);
            fdaHomePage.pressSearchEnter();
        ...
        // ---- Steps 18-23: Cart Validation ----
        LoggerUtility.info("Step 18: Clicking 'Mi carrito' cart icon");
        ...
        LoggerUtility.info("Step 20: Cart product names: " + cartProductNames);
        ...
        LoggerUtility.info("Step 21-22: Cart quantities: " + quantities);
        ...
        Assert.assertEquals(quantities.get(0), "2",
            TC_NAME + ": Product 1 cart quantity should be 2 (SKU " + TC_SKU_1 + "). Actual: " + quantities.get(0));
        Assert.assertEquals(quantities.get(1), "2",
            TC_NAME + ": Product 2 cart quantity should be 2 (SKU " + TC_SKU_2 + "). Actual: " + quantities.get(1));
        ...
        // ---- Steps 24-27: Checkout + PayPal Selection ----
        ...
        LoggerUtility.info("Step 26: Selecting 'PayPal' radio button");
        ...
        LoggerUtility.info("Step 27: Clicking 'PayPal Pagar' button");
        fdaPaymentPage.clickPayPalButton();

        // ---- Steps 28-39: PayPal Flow ----
        LoggerUtility.info("Step 28-29: Detecting and switching to PayPal popup window");
        switchToPayPalWindow(handlesBeforePayPal);
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        fdaPayPalPage.waitForPageLoad();
/*
        LoggerUtility.info("Step 30: Clicking PayPal email field");
        fdaPayPalPage.clickEmailField();

        LoggerUtility.info("Step 31: Entering PayPal email (masked in logs)");
        fdaPayPalPage.enterEmail(TC_PAYPAL_EMAIL);

        LoggerUtility.info("Step 32: Clicking 'Siguiente' on PayPal");
        fdaPayPalPage.clickNextButton();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 33: Clicking 'Contraseña' field on PayPal");
        fdaPayPalPage.clickPasswordField();

        LoggerUtility.info("Step 34: Entering PayPal password (masked in logs)");
        fdaPayPalPage.enterPassword(TC_PAYPAL_PASS);

        LoggerUtility.info("Step 35: Clicking 'Iniciar sesión' on PayPal");
        fdaPayPalPage.clickLoginButton();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);
*/
        LoggerUtility.info("Step 36: Clicking 'Compra completa' on PayPal review page");
        fdaPayPalPage.clickCompletePayment();
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 37-39: Waiting for PayPal window to close and switching back to FDA");
        waitForPayPalWindowClose();
        LoggerUtility.info("PayPal payment completed — switched back to FDA");

        // ---- Steps 40-41: Success Page ----
        LoggerUtility.info("Step 40: Verifying FDA order success page");
        ...
        LoggerUtility.info("Step 41: Order ID = " + orderId);
        ...
        // ---- Steps 42-46: Order History ----
        LoggerUtility.info("Step 42: Clicking 'Mi cuenta' profile icon");
        fdaHomePage.clickProfileIcon();

        LoggerUtility.info("Step 43: Clicking 'Mis pedidos' link");
        fdaHomePage.clickMyOrdersLink();

        LoggerUtility.info("Step 44: Verifying 'Mis pedidos' page is displayed");
        Assert.assertTrue(fdaOrderHistoryPage.isOrderHistoryDisplayed(),
            TC_NAME + ": Mis pedidos page not displayed");

        LoggerUtility.info("Step 45: Verifying Order ID " + orderId + " is present in order history");
        ...
        LoggerUtility.info("Step 46: FDA order status: " + fdaStatus);
        Assert.assertTrue(
        ...
        LoggerUtility.info("Step 47: Switching to Mirakl tab");
        switchToMiraklTab();
        driver.get(config.getMiraklUrl());
        ScreenshotUtility.captureScreenshot(driver, TC_NAME, ScreenshotUtility.INFO);

        LoggerUtility.info("Step 48: Clicking Orders menu in Mirakl");
        miraklOrdersPage.clickOrdersMenu();
        LoggerUtility.info("Step 49: Clicking All Orders");
        ...
        LoggerUtility.info("Step 50-51: Searching for order: " + miraklSearchTerm);
        boolean orderFoundInMirakl = false;
        for (int attempt = 1; attempt <= 10; attempt++) {
            LoggerUtility.info(TC_NAME + ": Mirakl search attempt " + attempt + "/10 for: " + miraklSearchTerm);
            ...
            LoggerUtility.info(TC_NAME + ": Order not in Mirakl yet (attempt " + attempt + "/10)");
            if (attempt < 10) {
        ...
        Assert.assertTrue(orderFoundInMirakl,
            TC_NAME + ": Order " + miraklSearchTerm + " did not appear in Mirakl within 10 minutes");

        // Steps 52-53: Verify 2 shipments
        LoggerUtility.info("Step 52-53: Verifying 2 shipments — WEB-A: " + shipmentRefA + " | WEB-B: " + shipmentRefB);
        ...
        // Step 54-56: Accept WEB-A
        LoggerUtility.info("Step 54: Opening and accepting WEB-A shipment: " + shipmentRefA);
        ...
        // Step 54-56: Accept WEB-B
        LoggerUtility.info("Step 55: Navigating back to accept WEB-B shipment: " + shipmentRefB);
        miraklOrdersPage.clickOrdersMenu();
        miraklOrdersPage.clickAllOrders();
        miraklOrdersPage.searchOrder(miraklSearchTerm);
        ...
        // Step 57: Wait 2 minutes
        LoggerUtility.info("Step 57: Waiting 2 minutes after acceptance for orders to stabilize in Kibo...");
        ...
        LoggerUtility.info("Step 58-60: ========== Kibo Authentication ==========");
        ...
        LoggerUtility.info("Step 61-63: ========== Find Kibo Order ID ==========");
        ...
        LoggerUtility.info("Step 64-66: ========== Polling Kibo Shipments (2 expected) ==========");
        ...
        for (int attempt = 1; attempt <= 20; attempt++) {
            ...
            if (shipmentList != null && shipmentList.size() >= 2) {
                ...
                if (allPopulated && ready.size() >= 2) {
                    // For TC_FBO_010 SKUs (78078094274 / 78078094107): items[0] → WEB-A
        ...
        // Steps 67-78: Process each shipment independently
        for (ShipmentInfo info : shipments) {
            LoggerUtility.info("Step 67: ========== Processing Shipment: " + info.shipmentRef + " ==========");
            LoggerUtility.info("Delivery Partner: " + info.deliveryPartner);
    ...
    private void runEnvioclickFlow(ShipmentInfo info) {
        LoggerUtility.info("Step 68: Calling Envioclick API — En tránsito for: " + info.shipmentRef);
        ...
        Assert.assertEquals(enTransito.getStatusCode(), 200,
            TC_NAME + ": Envioclick En tránsito should return 200 for shipment " + info.shipmentRef
            + ". Actual: " + enTransito.getStatusCode());
        LoggerUtility.info("EnvioClick En tránsito API Response = " + enTransito.getStatusCode());

        LoggerUtility.info("Step 69: Checking Mirakl for Shipped/3PL delivery status — " + info.shipmentRef);
        ...
        LoggerUtility.info("Step 70: Calling Envioclick API — Entregado for: " + info.shipmentRef);
        Response entregado = ApiUtility.postEnvioclickEntregado(
            info.tplShipmentId, info.trackingNumber, orderId, info.carrierName);
        Assert.assertEquals(entregado.getStatusCode(), 200,
            TC_NAME + ": Envioclick Entregado should return 200 for shipment " + info.shipmentRef
            + ". Actual: " + entregado.getStatusCode());
        LoggerUtility.info("EnvioClick Entregado API Response = " + entregado.getStatusCode());

        LoggerUtility.info("Step 71: Checking Mirakl for Received status — " + info.shipmentRef);
    ...
    private void runSkydropxFlow(ShipmentInfo info) {
        LoggerUtility.info("Step 72: Calling Skydropx API — Picked_up for: " + info.shipmentRef);
        ...
        Assert.assertEquals(pickedUp.getStatusCode(), 200,
            TC_NAME + ": Skydropx Picked_up should return 200 for shipment " + info.shipmentRef
            + ". Actual: " + pickedUp.getStatusCode());
        LoggerUtility.info("Skydropx Picked_up API Response = " + pickedUp.getStatusCode());

        LoggerUtility.info("Step 73: Checking Mirakl for Shipped status — " + info.shipmentRef);
        ...
        LoggerUtility.info("Step 74: Calling Skydropx API — Delivered for: " + info.shipmentRef);
        Response delivered = ApiUtility.postSkydropxDelivered(info.tplShipmentId);
        Assert.assertEquals(delivered.getStatusCode(), 200,
            TC_NAME + ": Skydropx Delivered should return 200 for shipment " + info.shipmentRef
            + ". Actual: " + delivered.getStatusCode());
        LoggerUtility.info("Skydropx Delivered API Response = " + delivered.getStatusCode());

        LoggerUtility.info("Step 75: Checking Mirakl for Received status — " + info.shipmentRef);
        status = waitForMiraklStatus("Received", 20);
